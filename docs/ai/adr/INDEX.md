...
| 008 | [Lock LFO — periodic modulation of the loop mutation rate](008-lock-lfo.md) | Proposed | Host-side LFO modulating `lock` per step. Shape (sine / tri / saw / sqr) × host-sync period (1/4..8 bars) × amount (0..1). Engine signature and `turing-test-vectors.json` unchanged — host pre-computes per-step lock from `base_lock + lfoSignal × amount`. New "Lock LFO" fieldset in vst RightRail + m4l mirror. v0.2 first ADR (chosen 2026-05-25 over per-bit density to avoid step-sequencer character). |
...
| 006 | [Stencil m4l v1 — release verification](archive/006-m4l-release-verification.md) | Implemented | Manual-Live verification for Stencil m4l v1 (live.* coverage, UI-scale/theme rendering, bit-ring interaction, trigger × output smoke, transport hung-note discipline, bake artifact hygiene) + per-channel distribution (GitHub Releases m4l-v0.1.1, maxforlive.com link-only listing 15366, im9.fm product page; demo on YouTube). §Verification + §Distribution ticked 2026-05-23; only the explicitly Optional second-machine cross-path test left unticked. |
